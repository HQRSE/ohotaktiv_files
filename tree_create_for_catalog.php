...
/* *** */
$glob = "/var/www/sibirix2/data/www/ohotaktiv.ru/obmen_files/quantity/catalog/Tree.xml";
$IBLOCK = 10;
	/* *** */
	$xml = new XMLReader();
...
/* LVL 1 */
		...
		while (count($assoc[0]['val'][$h_counter]['val']) > $parent_count_depth_0) {
			...
			if ($section_head_child_xml) {
				$trans = Cutil::translit($section_head_child_name,"ru",$arParams);
				/* search_id */
				...
				$arFields = Array(
   				...
   				if ($ar_result_l1 = $db_list->GetNext()) {
					...
					//$res = $bs->Update($ID, $arFields);
						...
						if ($section_head_child_id_l1 = $bs->Add($arFields)) {
							echo 'Сохранено ID: '.$section_head_child_id_l1;
						} else {
...
/* LVL 2 */
			...
			while (count($assoc[0]['val'][$h_counter]['val'][$parent_count_depth_0]['val']) > $parent_count_depth_1) {
				...
				if ($section_head_child_depth_1_xml) {
					$trans = Cutil::translit($section_head_child_depth_1_name,"ru",$arParams);
					/* search_id */
					...
					$arFields = Array(
						...
						"XML_ID" => $section_head_child_depth_1_xml,
						"CODE" => $trans
					);
   					if ($ar_result_l2 = $db_list->GetNext()) {
						...
						if ($section_head_child_id_l2 = $bs->Add($arFields)) {
							echo 'Сохранено ID: '.$section_head_child_id_l2;
						} else {
							echo 'Error: '.$bs->LAST_ERROR;
						}
					}
...
/* LVL 3 */
					...
					while (count($assoc[0]['val'][$h_counter]['val'][$parent_count_depth_0]['val'][$parent_count_depth_1]['val']) > $parent_count_depth_2) {
						...
						if ($section_head_child_depth_2_xml) {
							$trans = Cutil::translit($section_head_child_depth_2_name,"ru",$arParams);
							/* search_id */
							...
							$arFields = Array(
								...
								"XML_ID" => $section_head_child_depth_2_xml,
								"CODE" => $trans
							);
